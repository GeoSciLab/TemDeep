...
def evaluate_epoch(model, val_loader, block, args, epoch=0):
    ...
    v_mse, loss_values = evaluate(args, val_loader, model, args.val_num_interp, epoch, block)
...
def write_summary(global_index, learning_rate, t_loss,
    ...
    if args.rank == 0:
        ...
        args.logger.add_scalar("MSE", v_mse, global_index)
...
def save_model(model, optimizer, epoch, global_index, min_mse, block, args):
    ...
    if args.rank == 0:
        ...
        model_name = os.path.join(
            args.save_root, '_ckpt_epoch_%03d_iter_%07d_mse_%1.2f.pt.tar' % (
                epoch, global_index, min_mse))
...
def train(model, optimizer, lr_scheduler, train_loader,
    ...
    if args.initial_eval:
        ...
        v_mse, v_loss = evaluate_epoch(model, val_loader, block, args, args.start_epoch)
    else:
        v_mse, v_loss = 15.0, 0.0

    for epoch in range(args.start_epoch, args.epochs):

        # Train for an epoch.
        global_index = train_epoch(epoch, args, model, optimizer, lr_scheduler,
                                   train_sampler, train_loader, v_mse, v_loss, block)

        if (epoch + 1) % args.save_freq == 0:
            v_mse, v_loss = evaluate_epoch(model, val_loader, block, args, epoch + 1)
            if v_mse < min_mse:
                min_mse = v_mse
